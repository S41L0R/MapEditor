...
( function () {
	...
	Object.create = Object.create || ( function () {
		...
		return function ( base ) {
			...
			ctor.prototype = base || empty; return new ctor();
	...
	if ( ! window.JSON ) ( function () {
		...
		var e = void 0, i = ! 0, k = null, l = {}.toString, m, n, p = 'function' === typeof define && define.c, q = ! p && 'object' == typeof exports && exports; q || p ? 'object' == typeof JSON && JSON ? p ? q = JSON : ( q.stringify = JSON.stringify, q.parse = JSON.parse ) : p && ( q = this.JSON = {} ) : q = this.JSON || ( this.JSON = {} ); var r, t, u, x, z, B, C, D, E, F, G, H, I, J = new Date( - 3509827334573292 ), K, O, P; try {
...
q.parse( '{"A":[1,true,false,null,"\\u0000\\b\\n\\f\\r\\t"]}' ); if ( a = d.a.length == 5 && d.a[ 0 ] == 1 ) {
		...
		if ( ! R( 'json' ) ) {
			...
			J || ( K = Math.floor, O = [ 0, 31, 59, 90, 120, 151, 181, 212, 243, 273, 304, 334 ], P = function ( b, c ) {
				...
				var c = {}, a; if ( ( c.__proto__ = k, c.__proto__ = { toString: 1 }, c ).toString != l )m = function ( a ) {

					var b = this.__proto__, a = a in ( this.__proto__ = k, this ); this.__proto__ = b; return a;

				}; else {

					a = c.constructor; m = function ( b ) {

						var c = ( this.constructor || a ).prototype; return b in this && ! ( b in c && this[ b ] === c[ b ] );
...
k; return m.call( this, b );
			...
			n = function ( b, c ) {
				...
				var a = 0, d, j, f; ( d = function () {

					this.valueOf = 0;

				} ).prototype.valueOf = 0; j = new d(); for ( f in j )m.call( j, f ) && a ++; d = j = k; if ( a )a = a == 2 ? function ( a, b ) {

					var c = {}, d = l.call( a ) == '[object Function]', f; for ( f in a )! ( d && f == 'prototype' ) && ! m.call( c, f ) && ( c[ f ] = 1 ) && m.call( a, f ) && b( f );

				} : function ( a, b ) {

					var c = l.call( a ) == '[object Function]', d, f; for ( d in a )! ( c && d == 'prototype' ) && m.call( a, d ) && ! ( f = d === 'constructor' ) && b( d ); ( f || m.call( a, d = 'constructor' ) ) && b( d );

				}; else {

					j = [ 'valueOf', 'toString', 'toLocaleString',
						'propertyIsEnumerable', 'isPrototypeOf', 'hasOwnProperty', 'constructor' ]; a = function ( a, b ) {

						var c = l.call( a ) == '[object Function]', d; for ( d in a )! ( c && d == 'prototype' ) && m.call( a, d ) && b( d ); for ( c = j.length; d = j[ -- c ]; m.call( a, d ) && b( d ) );
				...
				var g = c[ b ], h, s, v, w, L, M, N, y, A; if ( typeof g == 'object' && g ) {
					...
					h = l.call( g ); if ( h == '[object Date]' && ! m.call( g, 'toJSON' ) ) if ( g > - 1 / 0 && g < 1 / 0 ) {
						...
						if ( P ) {
							...
							v = K( g / 864E5 ); for ( h = K( v / 365.2425 ) + 1970 - 1; P( h + 1, 0 ) <= v; h ++ );for ( s = K( ( v - P( h, 0 ) ) / 30.42 ); P( h, s + 1 ) <= v; s ++ );v = 1 + v - P( h, s ); w = ( g % 864E5 + 864E5 ) % 864E5; L = K( w / 36E5 ) % 24; M = K( w / 6E4 ) % 60; N = K( w / 1E3 ) % 60; w = w % 1E3;
...
'object' ) {
					...
					for ( b = o.length; b --; ) if ( o[ b ] === g ) throw TypeError(); o.push( g ); y = []; c = f; f = f + j; if ( h == '[object Array]' ) {

						s = 0; for ( b = g.length; s < b; A || ( A = i ), s ++ ) {

							h = x( s, g, a, d, j, f, o ); y.push( h === e ? 'null' : h );
						...
						n( d || g, function ( b ) {
							...
							var c = x( b, g, a, d, j, f, o ); c !== e && y.push( u( b ) + ':' + ( j ? ' ' : '' ) + c ); A || ( A = i );
...
typeof c == 'object' && c ) if ( l.call( c ) == '[object Function]' )j = c; else if ( l.call( c ) == '[object Array]' ) {
					...
					f = {}; o = 0; for ( g = c.length; o < g; h = c[ o ++ ], ( l.call( h ) == '[object String]' || l.call( h ) == '[object Number]' ) && ( f[ h ] = 1 ) );
				...
				if ( a ) if ( l.call( a ) == '[object Number]' ) {
					...
					if ( ( a = a - a % 1 ) > 0 ) {
						...
						d = ''; for ( a > 10 && ( a = 10 ); d.length < a; d = d + ' ' );

					}

				} else l.call( a ) == '[object String]' && ( d = a.length <= 10 ? a : a.slice( 0, 10 ) ); return x( '', ( h = {}, h[ '' ] = b, h ), j, f, d, '', [] );

			} ); R( 'json-parse' ) || ( z = String.fromCharCode, B = { '\\': '\\', '"': '"', '/': '/', b: '\u0008', t: '\t',
				n: '\n', f: '\u000c', r: '\r' }, C = function () {

				H = I = k; throw SyntaxError();
					...
					a = b.charAt( H ); if ( '\t\r\n '.indexOf( a ) > - 1 )H ++; else {
						...
						if ( a == '"' ) {
							...
							d = '@'; for ( H ++; H < c; ) {
								...
								a = b.charAt( H ); if ( a < ' ' )C(); else if ( a == '\\' ) {

									a = b.charAt( ++ H ); if ( '\\"/btnfr'.indexOf( a ) > - 1 ) {
										...
										j = ++ H; for ( f = H + 4; H < f; H ++ ) {
											...
											a = b.charAt( H ); a >= '0' && a <= '9' || a >= 'a' && a <= 'f' || a >= 'A' && a <= 'F' || C();
							...
							if ( a >= '0' && a <= '9' ) {
								...
								for ( a == '0' && ( a = b.charAt( H + 1 ), a >= '0' && a <= '9' ) && C(); H < c && ( a = b.charAt( H ), a >= '0' && a <= '9' ); H ++ );if ( b.charAt( H ) == '.' ) {

									for ( f = ++ H; f < c && ( a = b.charAt( f ), a >= '0' && a <= '9' ); f ++ );f == H && C(); H = f;

								}

								a = b.charAt( H ); if ( a == 'e' || a == 'E' ) {

									a = b.charAt( ++ H ); ( a == '+' || a == '-' ) && H ++; for ( f = H; f < c && ( a = b.charAt( f ), a >= '0' && a <= '9' ); f ++ );f == H && C(); H = f;
...
'false' ) {
				...
				var c, a; b == '$' && C(); if ( typeof b == 'string' ) {

					if ( b.charAt( 0 ) == '@' ) return b.slice( 1 ); if ( b == '[' ) {

						for ( c = []; ;a || ( a = i ) ) {

							b = D(); if ( b == ']' ) break; if ( a ) if ( b == ',' ) {

								b = D(); b == ']' && C();

							} else C(); b == ',' && C(); c.push( E( b ) );
					...
					if ( b == '{' ) {
						...
						for ( c = {}; ;a || ( a = i ) ) {
							...
							} else C(); ( b == ',' || typeof b != 'string' || b.charAt( 0 ) != '@' || D() != ':' ) && C(); c[ b.slice( 1 ) ] = E( D() );
			...
			G = function ( b, c, a ) {
				...
				a = F( b, c, a ); a === e ? delete b[ c ] : b[ c ] = a;

			}, F = function ( b, c, a ) {

				var d = b[ c ], j; if ( typeof d == 'object' && d ) if ( l.call( d ) == '[object Array]' ) for ( j = d.length; j --; )G( d, j, a ); else n( d, function ( b ) {
				...
				var a, d; H = 0; I = b; a = E( D() ); D() != '$' && C(); H = I = k; return c && l.call( c ) == '[object Function]' ? F( ( d = {}, d[ '' ] = a, d ), '', c ) : a;
