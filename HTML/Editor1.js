...
// DEPRECATED; USE EDITOR.JS INSTEAD!
...
const daeOnLoad = function (dae, currentUrl) {
	...
	*/
	...
	/*
	let daeMaterial;
	let daeGeometryArray;
	if (daeModel.children.length > 0) {
		for (let i = 1; i < daeModel.children.length; i++) {
			daeModel.children[i].geometry.computeBoundingSphere();
			console.log(daeModel.children[i].geometry);
			daeGeometryArray.push(daeModel.children[i].geometry.clone())
		}
	}
	else {
		daeModel.geometry.computeBoundingSphere();
		console.log(daeModel.geometry);
		daeGeometryArray.push(daeModel.geometry.clone())
		daeMaterial = daeModel.material.clone()
	}

	let daeGeometry = BufferGeometryUtils.mergeBufferGeometries(daeGeometryArray, false);

	daeModel = new THREE.InstancedMesh(daeGeometry, daeMaterial, 999)
	*/
	//scene.add(daeModel);
	//daeModel.parent.remove(daeModel.children[0]);
	console.log(daeModel)
...
function animate () {
	...
	*/
	console.warn("Scene polycount:", renderer.info.render.triangles)
	console.warn("Active Drawcalls:", renderer.info.render.calls)
