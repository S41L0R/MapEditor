...
namespace CafeLibrary
{
    ...
    public static class SARC_Parser
    {
        ...
        static SAHT[] GetHashTables()
        {
            ...
            if (Directory.Exists($"{Toolbox.Core.Runtime.ExecutableDir}/Hashes")) {
                foreach (var file in Directory.GetFiles($"{Toolbox.Core.Runtime.ExecutableDir}/Hashes"))
                {
                    if (Path.GetExtension(file) == ".saht")
        ...
        public static uint GuessAlignment(Dictionary<string, Stream> files)
        {
            uint res = 4;
        ...
        public static uint GuessFileAlignment(Stream stream)
        {
        ...
        public static byte[] pack(Dictionary<string, Stream> files, int align = -1, ByteOrder endianness = ByteOrder.LittleEndian) =>
        ...
        public static Dictionary<string, Stream> UnpackRam(byte[] src) =>
            UnpackRamN(new MemoryStream(src)).Files;

        [Obsolete("This has been kept for compatibility, use UnpackRamN instead")]
        public static Dictionary<string, Stream> UnpackRam(Stream src) =>
