...
{
    public class NUT : IFileFormat, ITextureContainer, IPropertyDisplay
    {
        ...
        private NUT_Parser Header { get; set; }

        public void Load(Stream stream)
        {
            Header = new NUT_Parser(stream);
        ...
        {
            private NUT_Parser.NutImage TextureHeader;

            public Texture(NUT_Parser.NutImage texture)
            {
                ...
                if (texture.Gx2HeaderData != null)
                {
                    var surface = texture.Gx2HeaderData;
                ...
                {
                    Platform = new DefaultSwizzle()
                    {
                        OutputFormat = texture.Format,
                    };
                }
            }
